...
export default function ScheduleSection({ isDark = false }: ScheduleSectionProps) {
	...
	const handleShare = async (post: ScheduledPost) => {
		...
		if (media) {
			...
			// No fallback: do not auto-download. Only copy caption and try share.
		} else {
		...
		<>
			...
			{showCopiedToast && (
				<div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
					<div className="bg-gray-900 border border-green-600 text-green-200 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-fade-in-up relative min-w-[220px] max-w-xs mx-auto pointer-events-auto">
						<span className="text-2xl">📋</span>
						<span className="font-semibold">Caption copied!</span>
						<button
							className="absolute top-2 right-2 text-green-400 hover:text-white text-lg p-1 rounded"
							...
							<div className="bg-gray-900 rounded-2xl p-6 w-full max-w-md mx-auto shadow-2xl relative">
								...
								<div className="space-y-4">
									...
									<textarea
										...
										className="w-full px-4 py-3 rounded-xl border bg-gray-800 border-gray-700 text-white placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all resize-y min-h-[80px]"
									/>
									{/* Time Picker */}
										...
										<div className="flex-1 min-w-0">
											<div className="flex items-center gap-2 mb-1">
												...
												{post.notificationSent && (
													<span className="ml-2 px-2 py-0.5 rounded bg-green-800 text-green-300 text-xs font-semibold">Notification sent</span>
												)}
											</div>
